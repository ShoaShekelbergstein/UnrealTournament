...
void UCommonNumericTextBlock::TimerTick()
{
...
bool UCommonNumericTextBlock::IsTickable() const
{
	// Needs to tick if it is interpolating anything and the update interval is 0.
	return CurrentInterpolationState != EInterpolationState::None && InterpolationUpdateInterval == 0.0f;
}

TStatId UCommonNumericTextBlock::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UCommonNumericTextBlock, STATGROUP_Tickables);
}
...
void UCommonNumericTextBlock::InterpolateToValue(const float InTargetValue, float InMaximumInterpolationDuration /*= 3.0f*/, float InMinimumChangeRate /*= 1.0f*/, float InOutroOffset /*= 0.0f*/)
{
	...
	UWorld* const World = GetWorld();
	if (InterpolationUpdateInterval != 0.0f && World)
	{
		if (CurrentInterpolationState != EInterpolationState::None)
		{
			LastTimerTickTime = World->GetTimeSeconds();
			World->GetTimerManager().SetTimer(TimerTickHandle, this, &ThisClass::TimerTick, InterpolationUpdateInterval, true);
		}
	}
}
...
{
	FText NumeralText = IsPercentage ? FText::AsPercent(InCurrentNumericValue, &InNumberFormattingOptions) : FText::AsNumber(InCurrentNumericValue, &InNumberFormattingOptions);
	return NumeralText;
}
