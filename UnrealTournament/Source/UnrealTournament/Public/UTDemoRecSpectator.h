...
class UNREALTOURNAMENT_API AUTDemoRecSpectator : public AUTPlayerController
{
	...
	virtual void DemoNotifyCausedHit(APawn* InstigatorPawn, AUTCharacter* HitPawn, uint8 AppliedDamage, FVector Momentum, const FDamageEvent& DamageEvent, bool bArmorDamage);
...
#if !UE_SERVER
	virtual void UpdateInputMode(bool bForce = false) override 
	{
		if (!IsKillcamSpectator())
		{
			Super::UpdateInputMode(bForce);
		}
