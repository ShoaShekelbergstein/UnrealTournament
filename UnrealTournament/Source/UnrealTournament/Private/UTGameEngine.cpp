...
void UUTGameEngine::Start()
{
	...
	// Now add any epic rulesets.  NOTE
	...
	if (IsRunningDedicatedServer())
	{
		...
		if (OnlineSubsystem) 
		{
			...
			{
				OnReadTitleFileCompleteDelegate = OnlineTitleFileInterface->AddOnReadFileCompleteDelegate_Handle(FOnReadFileCompleteDelegate::CreateUObject(this, &UUTGameEngine::OnReadTitleFileComplete));
				OnEnumerateTitleFilesCompleteDelegate = OnlineTitleFileInterface->AddOnEnumerateFilesCompleteDelegate_Handle(FOnEnumerateFilesCompleteDelegate::CreateUObject(this, &UUTGameEngine::OnEnumerateTitleFilesComplete));
...
void UUTGameEngine::OnEnumerateTitleFilesComplete(bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		IOnlineSubsystem* OnlineSubsystem = IOnlineSubsystem::Get();
		if (OnlineSubsystem) 
		{
			...
			if (OnlineTitleFileInterface.IsValid())
			{
				OnlineTitleFileInterface->ReadFile(UUTLocalPlayer::GetMCPGameRulesUpdateFilename());
				return;
			}
		}
	}
...
void UUTGameEngine::OnReadTitleFileComplete(bool bWasSuccessful, const FString& Filename)
{
	bReceivedTitleFiles = true;
...
void UUTGameEngine::ProcessMCPRulesetUpdate(FString MCPRulesetJson)
{
		...
		for (int32 i = 0; i < JsonGameRules.Rules.Num(); i++)
		{
			...
			for (int32 j=0; j < GameRulesets.Num(); j++)
			{
				if ( GameRulesets[j] == JsonGameRules.Rules[i] )
				{
					...
					GameRulesets[j].MaxMapsInList = JsonGameRules.Rules[i].MaxMapsInList;
					GameRulesets[j].EpicMaps = JsonGameRules.Rules[i].EpicMaps;
					GameRulesets[j].DefaultMap = JsonGameRules.Rules[i].DefaultMap;
					GameRulesets[j].QuickPlayMaps = JsonGameRules.Rules[i].QuickPlayMaps;
